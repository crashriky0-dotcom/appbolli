...
class PreventivoFormActivity : AppCompatActivity() {
    ...
    private fun recalcTotals() {
        ...
        // sconto effettivo vs prezzo reale
        val prezzoReale = findViewById<EditText>(R.id.edtPrezzoReale).text.toString().trim().toDoubleOrNull() ?: 0.0
        val diff = (totalConsigliato - prezzoReale)
        val perc = if (totalConsigliato > 0) (diff / totalConsigliato) * 100.0 else 0.0
